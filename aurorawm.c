 ...
 *
 * The event handlers of aurorawm are organized in an array which is accessed
...
static void manage_aurora_shell_bar(Window win, XWindowAttributes *wa);
...
static void unmanage_aurora_shell_bar(Window w);
...
void destroynotify(XEvent *e) {
    ...
    if ((c = wintoclient(ev->window)))
        ...
        unmanage_aurora_shell_bar(ev->window);
}
    ...
    if ((w = m->ww - tw - x) > bh) {
        if (m->sel) {
            ...
            drw_text(drw, x, 0, w, bh, lrpad / 2,
                     with_markup("<span font_family=\"Inter\">%s</span>",
                                 m->sel->name),
                     0, True);
            if (m->sel->isfloating)
...
void manage_aurora_shell_bar(Window win, XWindowAttributes *wa) {
    Monitor *m;
...
void maprequest(XEvent *e) {
    ...
    if (wmclasscontains(ev->window, aurora_shell_bar_class, ""))
        manage_aurora_shell_bar(ev->window, &wa);
...
void restoreSession(void) {
    ...
    char *str = malloc(
        23 *
        sizeof(
            char)); // allocate enough space for expected input from text file
    while (fscanf(fr, "%[^\n] ", str) != EOF) {
...
void scan(void) {
    ...
    if (XQueryTree(dpy, root, &d1, &d2, &wins, &num)) {
        for (i = 0; i < num; i++) {
            ...
            if (wmclasscontains(wins[i], aurora_shell_bar_class, ""))
                manage_aurora_shell_bar(wins[i], &wa);
...
void setup(void) {
    ...
    lrpad = drw->font->h;
    bh = enable_shell_integration ? 0 : drw->font->h + 2;
    updategeom();
    /* init atoms */
    ...
    XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
                    PropModeReplace, (unsigned char *)"aurorawm", 3);
...
void spawnbar(void) {
    if (*shell_integration_cmd)
        system(shell_integration_cmd);
}
...
void spawn(const Arg *arg) {
    if (fork() == 0) {
        ...
        die("aurorawm: execvp '%s' failed:", ((char **)arg->v)[0]);
    }
}
...
void unmanage_aurora_shell_bar(Window w) {
...
void updatebars(void) {
    ...
    XClassHint ch = {"aurorawm", "aurorawm"};
    for (m = mons; m; m = m->next) {
        if (m->barwin)
            continue;
...
void updatebarpos(Monitor *m) {
    m->wy = m->my;
    m->wh = m->mh;
    if (m->showbar || enable_shell_integration) {
        m->wh -= m->bh;
...
int xerror(Display *dpy, XErrorEvent *ee) {
    if (ee->error_code == BadWindow ||
        ...
        return 0;
    fprintf(stderr, "aurorawm: fatal error: request code=%d, error code=%d\n",
...
int xerrorstart(Display *dpy, XErrorEvent *ee) {
    die("aurorawm: another window manager is already running");
    return -1;
}
...
int main(int argc, char *argv[]) {
    if (argc == 2 && !strcmp("-v", argv[1]))
        die("aurorawm-" VERSION);
    else if (argc != 1)
        die("usage: aurorawm [-v]");
    if (!setlocale(LC_CTYPE, "") || !XSupportsLocale())
        fputs("warning: no locale support\n", stderr);
    if (!(dpy = XOpenDisplay(NULL)))
        die("aurorawm: cannot open display");
    checkotherwm();
    setup();
#ifdef __OpenBSD__
